# Brownfield Agent — grant-management

**Run ID:** n976seacam0cdaenbfxmr1fd1x83rc8y
**Project ID:** jd74kwkbpqsagbz408ef8y81en83rxpz
**Repo:** /Users/damonbodine/Factory512/builds/grant-management-20260325-231201/grant-management
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Grant Fit Analyzer — on grants page, paste a funder's RFP text, AI compares against org profile and current programs, returns fit score 1-10 with alignment points and gaps. 2) Proposal Narrative Generator — on application detail page, from grant data (program, target population, goals, budget, timeline) generate first-draft narrative sections: Need Statement, Project Description, Goals {{REQUEST}} Objectives, Evaluation Plan. 3) Budget Justification Writer — from budget line items, generate prose justification for each category explaining why each cost is necessary. 4) Grant Report Draft — from expenditure data + program outcomes + activity logs, generate progress/final report: Executive Summary, Activities, Outcomes, Budget Summary, Challenges. Each feature needs: Convex action, React component with loading states, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/grant-management-20260325-231201/grant-management. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n976seacam0cdaenbfxmr1fd1x83rc8y"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n976seacam0cdaenbfxmr1fd1x83rc8y"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n976seacam0cdaenbfxmr1fd1x83rc8y","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n976seacam0cdaenbfxmr1fd1x83rc8y","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n976seacam0cdaenbfxmr1fd1x83rc8y","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/grant-management-20260325-231201/grant-management`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Grant Fit Analyzer — on grants page, paste a funder's RFP text, AI compares against org profile and current programs, returns fit score 1-10 with alignment points and gaps. 2) Proposal Narrative Generator — on application detail page, from grant data (program, target population, goals, budget, timeline) generate first-draft narrative sections: Need Statement, Project Description, Goals {{REQUEST}} Objectives, Evaluation Plan. 3) Budget Justification Writer — from budget line items, generate prose justification for each category explaining why each cost is necessary. 4) Grant Report Draft — from expenditure data + program outcomes + activity logs, generate progress/final report: Executive Summary, Activities, Outcomes, Budget Summary, Challenges. Each feature needs: Convex action, React component with loading states, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n976seacam0cdaenbfxmr1fd1x83rc8y",
  "projectName": "grant-management",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Grant Fit Analyzer — on grants page, paste a funder's RFP text, AI compares against org profile and current programs, returns fit score 1-10 with alignment points and gaps. 2) Proposal Narrative Generator — on application detail page, from grant data (program, target population, goals, budget, timeline) generate first-draft narrative sections: Need Statement, Project Description, Goals {{REQUEST}} Objectives, Evaluation Plan. 3) Budget Justification Writer — from budget line items, generate prose justification for each category explaining why each cost is necessary. 4) Grant Report Draft — from expenditure data + program outcomes + activity logs, generate progress/final report: Executive Summary, Activities, Outcomes, Budget Summary, Challenges. Each feature needs: Convex action, React component with loading states, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/grant-management-20260325-231201/grant-management`
