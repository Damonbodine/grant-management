...
export function ApplicationCreateForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    try {
      await (createApplication as any)({
        title: title.trim(),
        grantId: grantId,
        leadWriterId: assignedWriterId || undefined,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Select value={grantId} onValueChange={(v) => setGrantId(v ?? "")}>
              ...
              <SelectContent>
                {grants?.map((g: typeof grants[number]) => (
                  <SelectItem key={g._id} value={g._id}>{(g as any).name ?? (g as any).title ?? g._id}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1">
            ...
            <Select value={assignedWriterId} onValueChange={(v) => setAssignedWriterId(v ?? "")}>
              <SelectTrigger id="assignedWriterId">
                <SelectValue placeholder="Assign a writer (optional)" />
              </SelectTrigger>
              <SelectContent>
